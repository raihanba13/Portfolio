...
program main
  ...
  ! this x has some garbage value, why is it happening?
  x = newton(p3, 2d0)
  write (*, "(A)", advance = "no") "try newton(p3, 2):", x
...
contains
  ...
  ! add your three functions here
   ...
   function polyEval(p, x) result(res)

      implicit none
      double precision, dimension(5), intent(in)  :: p 
      ! If I use the below line, it creats some small garbage value
      ! double precision, dimension(1:), intent(in)  :: p  
      ! p1'(x) = 0.464E-309x^4+12.0x^2-10.0x-3.00
      ! Are these two line not same?
   ...
   function polyDeriv(p) result(p_1st_derv)
      
      implicit none
      double precision, dimension(5), intent(in)  :: p 
      double precision, dimension(4) :: p_1st_derv 

      integer :: i

      do i = 1, 4, 1
         p_1st_derv(i) = p(i+1)*(i)
      end do
