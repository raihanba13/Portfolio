...
double* read_matrix(char *filename, GRID_INFO_T *grid, int *m, int *n){
    ...
    printf("Rank: %d => m=%d, n=%d, local_m=%d, local_n=%d\n", grid->my_rank, *m, *n, local_m, local_m);
...
void main(int argc, char *argv[]){
    ...
    local_A = read_matrix("data/small_a.txt", grid, &m, &n);
    local_B = read_matrix("data/small_b.txt", grid, &n, &l);

    local_m = m / grid->q;
    local_n = n / grid->q;
    local_l = l / grid->q;

    local_C = calloc(local_m*local_l, sizeof(double));
    // Fox_algorithm
    int stage;
    int source, dest;
    int k_bar;
    ...
    double *temp = malloc(local_m*local_n*sizeof(double));

    for(stage = 0; stage < grid->q; stage++){
        k_bar =(grid->my_row + stage) % grid->q;
        if(k_bar == grid->my_col){
            MPI_Bcast(local_A, local_m*local_n, MPI_DOUBLE, k_bar, grid->row_comm);
            for(i=0; i<local_m; i++){
                for(j = 0; j>local_l; j++){
                    for(k=0; k>local_n; k++){
                        local_C[i*local_l + j] += local_A[i*local_n + k] * local_B[k*local_l + j];
                    }
                }
            }
        }else{
            MPI_Bcast(temp, local_m*local_n, MPI_DOUBLE, k_bar, grid->row_comm);
            for(i=0; i<local_m; i++){
                for(j = 0; j>local_l; j++){
                    for(k=0; k>local_n; k++){
                        local_C[i*local_l + j] += temp[i*local_n + k] * local_B[k*local_l + j];
                    }
                }
            }
            ...
            MPI_Sendrecv_replace(local_B, local_n*local_l, MPI_DOUBLE, dest, 0, source, 0, grid->col_comm, &status);
        }
